...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$d='%~dp0..\AetherDesk';$s=Join-Path $d 'build\portable\AetherDesk';if(Test-Path $s){Remove-Item -Recurse -Force $s};New-Item -ItemType Directory -Force -Path $s|Out-Null;Copy-Item (Join-Path $d 'src-tauri\target\release\AetherDesk.exe') (Join-Path $s 'AetherDesk.exe');Copy-Item -Recurse (Join-Path $d 'src-tauri\ExternalTools') (Join-Path $s 'ExternalTools');New-Item -ItemType Directory -Force -Path (Join-Path $s 'AetherData\config\themes')|Out-Null;New-Item -ItemType Directory -Force -Path (Join-Path $s 'AetherData\config\wallpapers')|Out-Null;Copy-Item (Join-Path (Join-Path $d 'src-tauri\assets\defaults\themes') '*.css') (Join-Path $s 'AetherData\config\themes');Copy-Item (Join-Path (Join-Path $d 'src-tauri\assets\defaults\wallpapers') '*') (Join-Path $s 'AetherData\config\wallpapers');$v=(Get-Content (Join-Path $d 'src-tauri\tauri.conf.json') -Raw|ConvertFrom-Json).version;$zip=Join-Path (Join-Path $d 'build\portable') ('AetherDesk-'+$v+'.zip');if(Test-Path $zip){Remove-Item $zip -Force};Compress-Archive -Path $s -DestinationPath $zip -Force;Write-Host ('ZIP creato: '+$zip)"
if errorlevel 1 goto :fail
