...
// Command 8: Enable updates for a Lua-managed game by disabling all pinned setManifestid calls.
#[tauri::command]
fn enable_lua_game_updates(app_id: u32, steam_path: String) -> Result<String, String> {
    if steam_path.trim().is_empty() {
        return Err("Steam installation path is required".to_string());
    }

    let changed = LuaManifestPins::new(steam_path, app_id).enable_updates()?;
    Ok(format!("Updates enabled for App ID {}. {} manifest pin(s) disabled.", app_id, changed))
}

// Command 9: Remove a game from Aether's Lua library without touching installed game files.
#[tauri::command]
...
// Command 10: Clean common non-original crack/emulator helper files generated by fix workflows.
#[tauri::command]
fn clean_game_crack_files(game_path: String) -> Result<String, String> {
    if game_path.trim().is_empty() {
        return Err("Game path is not available. The game may not be installed.".to_string());
    }

    let root = std::path::PathBuf::from(game_path);
    if !root.is_dir() {
        return Err("Game path does not exist or is not a directory.".to_string());
    }

    let mut removed = 0usize;
    let safe_dirs = ["steam_settings", "ColdClientLoader", "Goldberg" ];
    ...
    if let Ok(entries) = std::fs::read_dir(&root) {
        for entry in entries.flatten() {
            ...
            if path.is_file() && name.to_ascii_lowercase().starts_with("launch") && name.to_ascii_lowercase().ends_with(".bat") {
                std::fs::remove_file(&path)
                    .map_err(|e| format!("Failed to remove {}: {}", path.display(), e))?;
                removed += 1;
            }
    ...
    Ok(format!("Cleaned {} generated crack/helper item(s). Original game binaries were not touched.", removed))
}
...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            enable_lua_game_updates,
            remove_lua_game_from_library,
            clean_game_crack_files,
