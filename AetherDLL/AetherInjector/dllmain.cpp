#include "pch.h"
// ---------------------------------------------------------------------------
// dwmapi.dll proxy / injector
//
// Steam loads the system dwmapi.dll early at startup. By placing our own
// dwmapi.dll next to steam.exe we get loaded first; we then load AetherCore.dll
// into the process and forward every real dwmapi export to the system original
// (resolved through Windows DLL search/KnownDLLs) so Windows compositing keeps working unchanged.
//
// Required files in the Steam folder:
//   <Steam>\dwmapi.dll        this proxy (built x64)
//   <Steam>\AetherCore.dll    the main hook library
//
// Design notes:
//   * x64 only — Steam is 64-bit, a 32-bit proxy would never be loaded.
//   * We inject ONLY into steam.exe, never into games or other hosts that may
//     happen to load dwmapi.dll.
//   * AetherCore.dll is loaded by ABSOLUTE path (next to this proxy) so a change
//     in the process working directory cannot break the load; we fall back to a
//     name-only load if the path cannot be built.
//   * No heavy work in DllMain: a single LoadLibrary is fine and returns fast;
//     AetherCore does its own off-loader-lock initialisation internally.
// ---------------------------------------------------------------------------
#include "pch.h"
...
// ---------------------------------------------------------------------------
// Export forwarding to the system original dwmapi.dll.
//
// Keep the proxy's export surface close to the system dwmapi.dll. The earlier
// rework forwarded only the commonly used public DWM APIs; LumaCore carried a
// fuller list, including COM helpers, private Dwmp* symbols, and ordinal-only
// exports. Missing exports can make the loader reject the proxy on Windows/Steam
// builds that import one of the omitted symbols, so we forward the complete
// observed set to DWMAPI.dll.
// ---------------------------------------------------------------------------
...
// Ensures the injection runs exactly once, even if DllMain is re-entered.
volatile LONG s_injected = 0;

// Loads AetherCore.dll from the same directory as this proxy module, falling
// back to a name-only load if the absolute path cannot be built.
void LoadAetherCore(HMODULE self) {
    char path[MAX_PATH] = {};
    DWORD pathLen = GetModuleFileNameA(self, path, MAX_PATH);
    // GetModuleFileNameA returns 0 on failure; if it returns MAX_PATH the buffer
    // may be truncated and not null-terminated. In either case fall back to a
    // name-only load because we cannot reliably compute the AetherCore.dll path.
    if (pathLen == 0 || pathLen >= MAX_PATH) {
        LoadLibraryA("AetherCore.dll");
        return;
    }
    path[MAX_PATH - 1] = '\0';
    PathRemoveFileSpecA(path);  // strip "dwmapi.dll", leaving the Steam folder

    std::string full = std::string(path) + "\\AetherCore.dll";
    if (!LoadLibraryA(full.c_str())) {
        LoadLibraryA("AetherCore.dll");  // fallback if the absolute load failed
    }
}
...
BOOL APIENTRY DllMain(HMODULE instance, DWORD reason, LPVOID /*reserved*/) {
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        if (InterlockedCompareExchange(&s_injected, 1, 0) == 0) {
            // Resolve the host executable name and only inject into Steam itself.
            char exePath[MAX_PATH] = {};
            GetModuleFileNameA(nullptr, exePath, MAX_PATH);
            const char* exeName = PathFindFileNameA(exePath);

            if (_stricmp(exeName, "steam.exe") == 0) {
                LoadAetherCore(instance);
            }
        }
    }
    return TRUE;
